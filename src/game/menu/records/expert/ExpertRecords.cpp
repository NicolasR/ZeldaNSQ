...
ExpertRecords::ExpertRecords() : line(3), full(false), ulti(false), rush(false), surface(0), surfaceFull(0), surfaceUlti(0), surfaceRush(0), skin(0) {
...
void ExpertRecords::init() {
    ...
    Text* text = TextManager::getInstance()->getText(699);
    ...
    int time = globalSave->getBestExpertTime();
    if (time == -1) {
        text = new Text("??:??:??");
    } else {
        ostringstream os;
    ...
    delete text;
    surface->put(rang, 280, 56, globalSave->isExpertFull() ? 0 : 16, 0, 16, 16);

    // ultime
    text = TextManager::getInstance()->getText(702);
    text->displayOnSurface(44, 104, surface);
    delete text;
    surface->put(rang, 280, 104, globalSave->isExpertUlti() ? 0 : 16, 0, 16, 16);

    // rapidite
    text = TextManager::getInstance()->getText(703);
    text->displayOnSurface(44, 152, surface);
    delete text;
    surface->put(rang, 280, 152, globalSave->isExpertRush() ? 0 : 16, 0, 16, 16);
...
void ExpertRecords::cadre(int x, int y, int w, int h, Surface* surf) {
...
void ExpertRecords::handleEvents(Event* event) {

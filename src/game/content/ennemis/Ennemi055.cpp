    ...
    if (dist <= maxDist) {
        if (link->getStatus()->isVisible()) {
            ...
            if (link->getBoundingBox()->intersect(getBoundingBox())) {
                testDegatOnLink(&box, direction, 12, TA_PHYSIC, TE_NORMAL);
            }
        } else {
            idle = true;
            testDegatOnLink(getBoundingBox(), direction, 12, TA_PHYSIC, TE_NORMAL);
        }
        BoundingBox weapon;
        ...
        testDegatOnLink(&weapon, direction, 12, TA_PHYSIC, TE_NORMAL);
